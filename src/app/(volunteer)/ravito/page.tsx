...
import { Search, QrCode, CheckCircle, User, Utensils } from 'lucide-react';
...
export default function RavitoPage() {
  ...
  const distributeMutation = useMutation({
    mutationFn: (registrationId: string) =>
      api.post(`/registrations/${registrationId}/distributions`, { itemType: 'RAVITO' }),
    onSuccess: () => {
      toast.success('Ravito issued.');
      if (selected) lookupById(selected.id);
      queryClient.invalidateQueries({ queryKey: ['ravito-regs', raceId] });
    },
    onError: () => toast.error('Could not issue ravito.'),
  });

  const revokeMutation = useMutation({
    mutationFn: (registrationId: string) =>
      api.delete(`/registrations/${registrationId}/distributions/RAVITO`),
    onSuccess: () => {
      toast.success('Ravito revoked.');
      if (selected) lookupById(selected.id);
      queryClient.invalidateQueries({ queryKey: ['ravito-regs', raceId] });
    },
    onError: () => toast.error('Could not revoke ravito.'),
  });

  const ravitoIssued = selected?.distributions?.some((d: { itemType: string }) => d.itemType === 'RAVITO');
          ...
          <div className="flex flex-col border border-gray-200 rounded-lg bg-white overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
              {listLoading ? (
                ...
                const issued = r.distributions?.some((d: { itemType: string }) => d.itemType === 'RAVITO');
                return (
                  <button
                    key={r.id}
                  ...
                  >
                    ...
                    <div className="pl-5 mt-0.5">
                      ...
                      <span className={`text-xs font-medium ${issued ? 'text-green-500' : 'text-gray-400'}`}>
                ...
                {ravitoIssued ? (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700 font-medium">
                      <CheckCircle className="h-4 w-4" /> Ravito already issued
                    </div>
                    <button
                  ...
                  >
                    ...
                    {distributeMutation.isPending ? 'Issuing…' : 'Issue Ravito'}
                  </Button>
                )}
              </CardContent>
            </Card>
